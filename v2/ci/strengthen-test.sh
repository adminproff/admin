...
''',
    '''for command_name in \\
    qemu-system-x86_64 qemu-img xorriso unsquashfs socat python3 curl grep sed awk timeout jq; do
''',
    "проверка наличия test-инструментов",
)
...
''',
    "установка с USB и safety marker",
)
